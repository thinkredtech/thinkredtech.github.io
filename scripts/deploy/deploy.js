...
function deployBackend() {
  ...
  if (!fs.existsSync(clasprcPath)) {
    console.log(chalk.yellow('⚠️ clasp not configured. Please run "clasp login" and "clasp create" first.'));
    ...
    console.log(chalk.yellow('📝 Created .clasp.json template. Please update with your actual script ID.'));
  ...
  try {
    ...
    execCommand('clasp deploy --description "Automated deployment"', { cwd: backendPath });
  }
...
function main() {
  ...
  if (args.length === 0) {
    console.log(chalk.yellow('🤔 No component specified. Deploying all components...'));
    deployAll();
    return;
  }
