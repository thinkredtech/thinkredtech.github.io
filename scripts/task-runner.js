...
async function runParallelCommands(commands) {
  const promises = commands.map(
    ({ description, command, workspace, options }) =>
...
function showHelp() {
  ...
  Object.entries(categories).forEach(([category, tasks]) => {
    ...
    tasks.forEach((taskName) => {
      ...
      if (task && !task.alias) {
        const workspaces = task.workspaces
          .map((ws) => WORKSPACES[ws].icon)
          .join(" ");
        console.log(
          `  ${chalk.green(taskName.padEnd(20))} ${workspaces} ${task.description}`,
        );
      }
    });
    console.log();
  ...
  console.log(
    "  npm run task dev                 # Start frontend development",
  );
  console.log(
    "  npm run task dev:all              # Start all development servers",
  );
...
function showWorkspaceStatus() {
  ...
  Object.entries(WORKSPACES).forEach(([key, ws]) => {
    ...
    if (pkg) {
      ...
      console.log(
        `   Scripts: ${Object.keys(pkg.scripts || {}).length} available`,
      );
    } else {
...
async function executeTask(taskName, targetWorkspace = null, options = {}) {
  ...
  try {
    ...
    if (task.parallel) {
      // Run in parallel
      const commands = workspaces
        .map((workspace) => {
          const script = task.scripts ? task.scripts[workspace] : task.script;
          if (!script) {
            logWarning(
              `No script defined for ${workspace} in task ${taskName}`,
            );
            return null;
          }
          ...
          if (!hasScript(workspace, script)) {
            logWarning(
              `Script '${script}' not found in ${workspace} package.json`,
            );
            return null;
          }
