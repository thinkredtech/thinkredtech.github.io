...
function readPackageJson(component) {
  ...
  if (!fs.existsSync(packagePath)) {
    throw new Error(
      `Package.json not found for ${component} at ${packagePath}`,
    );
  }
...
function updateVersion(component, releaseType = "patch") {
  ...
  if (!semver.valid(currentVersion)) {
    throw new Error(
      `Invalid version format in ${component}: ${currentVersion}`,
    );
  }
  ...
  console.log(
    chalk.green(
      `✅ Successfully updated ${component} version to ${newVersion}`,
    ),
  );
...
function showCurrentVersions() {
  ...
  VALID_COMPONENTS.forEach((component) => {
    try {
      ...
      console.log(
        chalk.red(`  ${component}: Error reading version - ${error.message}`),
      );
    }
  });
}
...
function main() {
  ...
  if (!VALID_RELEASE_TYPES.includes(releaseType)) {
    ...
    console.error(
      chalk.yellow(`Valid release types: ${VALID_RELEASE_TYPES.join(", ")}`),
    );
    process.exit(1);
  }
