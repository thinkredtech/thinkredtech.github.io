# Developer Documentation

Welcome to the ThinkRED developer documentation hub! This section contains comprehensive guides, references, and resources for developers working on the ThinkRED project.

## 📚 Documentation Sections

### 🚀 [Getting Started](setup/)

- **[Installation Guide](setup/installation.md)** - Set up your development environment
- **[Environment Configuration](setup/environment.md)** - Configure your local environment
- **[Troubleshooting](setup/troubleshooting.md)** - Common issues and solutions

### 📖 [Development Guides](guides/)

- **[Development Workflow](guides/development.md)** - Day-to-day development practices
- **[Contributing Guidelines](guides/contributing.md)** - How to contribute to the project
- **[Configuration Management](guides/configuration.md)** - Managing project configurations
- **[Code Style Guide](guides/code-style.md)** - Coding standards and best practices
- **[Documentation Guidelines](guides/documentation-contribution.md)** - How to write and maintain docs
- **[Accessibility Guidelines](guides/accessibility.md)** - Making the app accessible to all users

### 🏗️ [Architecture](architecture/)

- **[System Overview](architecture/system-overview.md)** - High-level system architecture
- **[Frontend Architecture](architecture/frontend-architecture.md)** - React app structure and patterns

- **[Backend Architecture](architecture/backend-architecture.md)** - Google Apps Script backend design

### 🚀 [Deployment](deployment/)

- **[Production Deployment](deployment/production.md)** - Complete deployment guide

- **[CI/CD Pipeline](deployment/cicd.md)** - Automated deployment workflows
- **[Environment Management](deployment/environments.md)** - Managing different environments

### 🔌 [APIs](apis/)

- **[Backend APIs](apis/backend-apis.md)** - Complete API reference
- **[Frontend Integration](apis/frontend-integration.md)** - How to use APIs in the frontend
- **[Authentication](apis/authentication.md)** - API authentication patterns

## 🎯 Quick Links

| Resource | Description |
|----------|-------------|
| [Installation](setup/installation.md) | Get up and running quickly |
| [Development Workflow](guides/development.md) | Start developing features |
| [Production Deployment](deployment/production.md) | Deploy to production |
| [API Reference](apis/backend-apis.md) | Backend API documentation |
| [Contributing](guides/contributing.md) | How to contribute |

## 🆘 Need Help?

- 📖 Check the [Troubleshooting Guide](setup/troubleshooting.md)
- 💬 Ask questions in our development discussions
- 🐛 Report issues on GitHub
- 📧 Contact the development team

## 📝 Contributing to Documentation

Found an issue or want to improve the docs? See our [Documentation Guidelines](guides/documentation-contribution.md) for how to contribute.

---

<div align="center">

### 🚀

 Ready to build something amazing? Let's get started

[![Installation Guide](https://img.shields.io/badge/📦%20Installation-Get%20Started-blue?style=for-the-badge)](setup/installation.md)
[![Development Guide](https://img.shields.io/badge/💻%20Development-Learn%20More-green?style=for-the-badge)](guides/development.md)

</div>
